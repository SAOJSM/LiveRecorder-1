...
async def get_douyu_stream_data(rid: str, rate: str = '-1', proxy_addr: OptionalStr = None,
    ...
    data = {
        ...
        'did': params_list[1],
        'tt': params_list[2],
        'sign': params_list[3],  # 10分鐘有效期
        'ver': '22011191',
        'rid': rid,
        'rate': rate,  # 0藍光、3超清、2高清、-1預設
    }
...
async def get_bilibili_stream_data(url: str, qn: str = '10000', platform: str = 'web', proxy_addr: OptionalStr = None,
    ...
    if json_data and json_data['code'] == 0:
        ...
        # 此介面因網頁上有限制, 需要配置登錄后的cookie才能獲取最高畫質
        ...
        # qn: 30000=杜比 20000=4K 10000=原畫 400=藍光 250=超清 150=高清 80=流暢
...
async def get_bigo_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    if live_status == 1:
        ...
        result['anchor_name'] = re.search('<title>歡迎來到(.*?)的直播間</title>', html_str, re.DOTALL).group(1)
    ...
    try:
        ...
        if login_need:
            ...
            if len(username) < 6 or len(password) < 8:
                raise RuntimeError("FlexTV登錄失敗！請在config.ini配置檔案中填寫正確的FlexTV平臺的帳號和密碼")
...
def get_looklive_secret_data(text) -> tuple:
    # 本演算法參考專案：https://github.com/785415581/MusicBox/blob/b8f716d43d/doc/analysis/analyze_captured_data.md
...
async def get_looklive_stream_url(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    """
    通過PC網頁端的介面獲取完整直播源，只有params和encSecKey這兩個加密請求參數。
    params: 由兩次AES加密完成
    ncSecKey: 由一次自寫的加密函式完成，值可固定
    """
...
async def get_huajiao_user_info(url: str, cookies: OptionalStr = None, proxy_addr: OptionalStr = None) -> OptionalDict:
    ...
    if 'user' in url:
        ...
        anchor_name = re.search('<title>(.*?)的主頁.*</title>', html_str).group(1)
