...
def dingtalk(url: str, content: str, number: str = None, is_atall: bool = False) -> Dict[str, Any]:
    ...
    for api in api_list:
        ...
        try:
            ...
            error.append(api)
            print(f'釘釘推送失敗, 推送地址：{api}, 錯誤資訊:{e}')
    ...
    for api in api_list:
        ...
        try:
            ...
            if json_data['code'] == 200:
                ...
                error.append(api)
                print(f'微信推送失敗, 推送地址：{api}, 失敗資訊：{json_data["msg"]}')
        except Exception as e:
            error.append(api)
            print(f'微信推送失敗, 推送地址：{api}, 錯誤資訊:{e}')
    return {"success": success, "error": error}
...
def send_email(email_host: str, login_email: str, email_pass: str, sender_email: str, sender_name: str,
    ...
    try:
        ...
        print(f'郵件推送失敗, 推送郵箱：{to_email}, 錯誤資訊:{e}')
...
def tg_bot(chat_id: int, token: str, content: str) -> Dict[str, Any]:
    try:
        ...
        print(f'tg推送失敗, 聊天ID：{chat_id}, 錯誤資訊:{e}')
    ...
    for _api in api_list:
        ...
        try:
            ...
            if json_data['code'] == 200:
                ...
                error.append(_api)
                print(f'Bark推送失敗, 推送地址：{_api}, 失敗資訊：{json_data["message"]}')
        except Exception as e:
            error.append(api)
            print(f'Bark推送失敗, 推送地址：{_api}, 錯誤資訊:{e}')
    return {"success": success, "error": error}
    ...
    for _api in api_list:
        ...
        try:
            ...
            if "error" not in json_data:
                ...
                error.append(_api)
                print(f'ntfy推送失敗, 推送地址：{_api}, 失敗資訊：{json_data["error"]}')
        except urllib.error.HTTPError as e:
            error.append(_api)
            error_msg = e.read().decode("utf-8")
            print(f'ntfy推送失敗, 推送地址：{_api}, 錯誤資訊:{json.loads(error_msg)["error"]}')
        except Exception as e:
            error.append(api)
            print(f'ntfy推送失敗, 推送地址：{_api}, 錯誤資訊:{e}')
    return {"success": success, "error": error}
...
if __name__ == '__main__':
    ...
    # 釘釘推送通知
    webhook_api = ''  # 替換成自己Webhook鏈接,參考文件：https://open.dingtalk.com/document/robots/custom-robot-access
    phone_number = ''  # 被@使用者的手機號碼
    is_atall = ''  # 是否@全體
    # dingtalk(webhook_api, send_content, phone_number)

    # 微信推送通知
    # 替換成自己的單點推送介面,獲取地址：https://xz.qqoq.net/#/admin/one
    # 當然也可以使用其他平臺API 如server醬 使用方法一樣
    xizhi_api = 'https://xizhi.qqoq.net/xxxxxxxxx.send'
    # xizhi(xizhi_api, send_content)

    # telegram推送通知
    tg_token = ''  # tg搜索"BotFather"獲取的token值
    tg_chat_id = 000000  # tg搜索"userinfobot"獲取的chat_id值，即可發送推送訊息給你自己，如果下面的是群組id則發送到群
